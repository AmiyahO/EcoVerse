...
**2. Daily Step Summaries** — `Steps` records written to HC by any app (Samsung Health, Google Fit, the phone's OS step counter) are aggregated per local calendar day. This captures steps from users who walk with their phone but don't start a tracked workout. `fetchDailyStepSummaries()` produces one importable walking entry per day.

> **Cross-app double-write deduplication:** Multiple apps (e.g. Samsung Health and Google Fit) often write identical `Steps` records to HC for the same physical steps — Google Fit reads from HC and writes back, so a walk of 3,601 steps can appear as two separate records totalling 7,202. `fetchDailyStepSummaries()` handles this by bucketing steps by `(localDate, dataOrigin)` and then taking the **maximum single-origin total per day** rather than summing across all origins. `max(3601, 3601) = 3601`. Distance records receive the same treatment. Samsung Health's own UI may also show a higher count than HC because it applies its own sensor-fusion algorithm on top of the raw HC records — this gap is a platform limitation EcoVerse cannot close.
